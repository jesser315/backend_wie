...
def get_ai_recommendation(api_key, data_string):
    ...
    # This is the exact prompt instruction you provided
    user_prompt_instruction = (
        "Based on the following forecast data, please provide very simple sentence "
        "in the tunisian dialect about the weather , the vegetables i plant are onions "
        "tomatoes and mint ,i need you to tell me which one i should water and if i "
        "should water or not(put in your mind that some vegetables need more water "
        "than others and put in mind the day of the year) , just say it in a "
        "single sentence also in the tunisian dialect "
    )
    
    # This system prompt helps guide the AI's tone and constraints
    system_prompt = (
        "You are a helpful farmer's assistant. You must respond in the "
        "Tunisian dialect (Darja) and you must provide the answer "
        "in only one single, simple sentence."
    )
    ...
    try:
        response = client.chat.completions.create(
            ...
            max_tokens=150,  # Enough for a couple of sentences
            temperature=0.5  # A bit of creativity but mostly factual
        )
